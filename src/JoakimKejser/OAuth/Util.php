...
class Util
{
	...
	{
		if (is_array($input)) {
			$urlencode = array_map(['JoakimKejser\OAuth\Util', 'urlencodeRfc3986'], $input);

			return $urlencode;
		}
		if (is_scalar($input)) {
			$urlencode = str_replace('+', ' ', str_replace('%7E', '~', rawurlencode($input)));
	...
	public static function splitHeader($header, $onlyAllowOAuthParameters = true)
	{
		$params = [];
		if (preg_match_all(
			'/(' . ($onlyAllowOAuthParameters ? 'oauth_' : '') . '[a-z_-]*)=(:?"([^"]*)"|([^,]*))/',
			$header,
			$matches
		)) {
